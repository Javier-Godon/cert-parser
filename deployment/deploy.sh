...
if [[ -z "${DRY_RUN}" ]]; then
    ...
    echo ""
    echo "  ── Via port-forward (bypasses Ingress, no prefix needed) ──"
    ...
    echo ""
    echo "  ── Via API gateway (uses /prt-cert-parser prefix from Ingress) ──"
    echo "  curl http://<gateway-host>/prt-cert-parser/health"
    echo "  curl http://<gateway-host>/prt-cert-parser/ready"
    echo "  curl -X POST http://<gateway-host>/prt-cert-parser/trigger"
    echo ""
